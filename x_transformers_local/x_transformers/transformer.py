...
class Attention(nn.Module):
    def __init__(
            ...
            # new variables...
            kernel_ratio=0.5,
            attn_type='self_attn'  # options: 'self_attn', 'linear_attn', 'linformer_attn', 'performer_attn'
    ):
        super().__init__()

        # set attn type
        self.attn_type = attn_type
        # linear attn functions
        self.k_feature_map = nn.ELU()
        self.q_feature_map = nn.ELU()
    ...
    def forward(
        ...
        elif self.attn_type == 'linear_attn':
            # note: referenced the linear attention implementation from the original authors in the fast-transformers repo
            # to better understand tensor shapes and the order/structure of batch matrix multiplication steps
            # https://github.com/idiap/fast-transformers/blob/master/fast_transformers/attention/linear_attention.py

            # note: definition of dimensions...
            # k      = [B,H,J,D] = batch, heads, seq len, k dims
            # v, out = [B,H,J,E] = batch, heads, seq len, v dims
            # kv     = [B,H,E,D] = batch, heads, v dims,  k dims
            # q      = [B,H,I,D] = batch, heads, seq len, k dims
            # z      = [B,H,I]   = batch, heads, seq len

            # apply feature map
            q_mapped = self.q_feature_map(q) + 1
            k_mapped = self.k_feature_map(k) + 1

            # calculate KV matrix
            kv = torch.einsum('bhjd,bhje->bhed', k_mapped, v)
            # kv = torch.einsum('nhsd,nhsm->nhmd', k_mapped, v)

            # calculate normalizer (denominator)
            k_sum = k_mapped.sum(
                dim=2)  # sum of keys in the 3rd dimension (sequence), (input dims: bhnd, output dims: bhd)
            z = 1.0 / (torch.einsum('bhid,bhd->bhi', q_mapped,
                                    k_sum) + 1e-6)  # calculate the normalizer (adding 1e-6 prevents divide by zero errors)
            # z = 1.0 / (torch.einsum('nhld,nhd->nhl', q_mapped, k_sum) + 1e-6)

            # calculate the output
            out = torch.einsum('bhid,bhed,bhi->bhie', q_mapped, kv, z)
            # out = torch.einsum('nhld,nhmd,nhl->nhlm', q_mapped, kv, z)

            # # scale the attention... not sure if this is necessary for linear attention since it's already normalized - AJM ??
            # if head_scale:
            #     out = out * self.head_scale_params

            # rearrange to match the shape the repo is expecting
            out = rearrange(out, 'b h n e -> b n (h e)')

            # linear attention doesn't have the same intermediates (no softmax), but I initialized this as empty to prevent errors - AJM
            intermediates = Intermediates(
                qk_similarities=None,
                pre_softmax_attn=None,
                post_softmax_attn=None
            )

            # nonlinear tranform on out
            out = self.to_out(out)

        elif self.attn_type == 'performer_attn':
            # note: referenced the minimal performer implementation to understand tensor structure and
            # batch matrix multiplication steps:
            # https://github.com/cloneofsimo/smallest_working_performer/blob/main/model.py#L35

            # Need to finish this ... AJM
...
class AttentionLayers(nn.Module):
    def __init__(
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type == 'a':
                layer = Attention(dim, heads=heads, causal=causal, attn_type=self.attn_type, **attn_kwargs)
            elif layer_type == 'c':
                layer = Attention(dim, heads=heads, attn_type=self.attn_type, **attn_kwargs)
            elif layer_type == 'f':
