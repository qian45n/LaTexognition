...
def get_encoder(args):
    if args.encoder_type.lower() == 'resnet':
        backbone = ResNetV2(
    ...
    elif args.encoder_type.lower() == 'efficientnet':
        #backbone = EfficientNet(num_classes=0, in_chans=args.channels)
        model_name = 'efficientnet_b0'  # You can change this to the desired EfficientNet variant (e.g., 'efficientnet_b1', 'efficientnet_b2', etc.)
        #backbone = EfficientNet.from_name(model_name, pretrained=False, num_classes=0, in_chans=args.channels)
        #backbone = timm.create_model('tf_efficientnet_b4_ns', pretrained=True, num_classes=0)
        backbone = timm.create_model('resnet34', pretrained=True, in_chans=1, num_classes=0)
        min_patch_size = 2**(len(args.backbone_layers)+1)
