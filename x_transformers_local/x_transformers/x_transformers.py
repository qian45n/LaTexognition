...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.linear_attn:
            ...
            #     print(input_mask.shape)
            #     print('k:{}, v:{}, q:{}'.format(k.shape, v.shape, q.shape))
            #     kv_mask = input_mask[:, None, None, :]
            #     kv_mask_value = max_neg_value(q)
            #     k = k.masked_fill(~kv_mask, kv_mask_value)
            #     v = v.masked_fill(~kv_mask, 0.)
            #     del kv_mask
            dots = einsum('bhnd,bhne->bhde', k, v)
            # mask_value = max_neg_value(dots)
            # if exists(context_mask):
            #     dots = dots.masked_fill(~rearrange(context, 'b h  -> b 1 1 j'), mask_value)
        ...
        if exists(input_mask) and not self.linear_attn:
            input_mask = rearrange(input_mask, 'b j -> b 1 1 j')
        ...
        if self.linear_attn:
            out = einsum('b h i d, b h d e -> b h i e', q, attn)
        else:
        ...
        #if exists(mask):
        if exists(mask) and not self.linear_attn:
    ...
    def __init__(
        ...
        **kwargs
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type == 'a':
                layer = Attention(dim, heads = heads, causal = causal, linear_attn=self.linear_attn, **attn_kwargs)
            elif layer_type == 'c':
                layer = Attention(dim, heads = heads, linear_attn=self.linear_attn, **attn_kwargs)
            elif layer_type == 'f':
...
# class SelfAttention(AttentionLayers):
#     def __init__(self, dim, heads, dim_head = None, n_local_attn_heads = 0, local_attn_window_size = 16, receives_context = False, dropout = 0., attn_dropout = 0.):
#         super().__init__()
...
#         self.local_attn  = LocalAttention(local_attn_window_size, dropout = attn_dropout)
